...
Therefore, we will be using the median of 21 degrees Celsius in calculations. I added 
5 degrees to these values to account for circuit heat.
Setting 1: 18 C
Setting 2: 19 C
Setting 3: 22 C
Setting 4: 23 C
...
ALSO INCLUDED MANUAL INPUT ENDPOINTS FROM APP.
ManualOne, ManualTwo, ... ManualFive
*/
      ...
      if (client.available()) {             
        ...
        if (c == '\n') {                    // check for end of HTTP request
          if (currentLine.length() == 0) {
            ...
            // MANUAL MODES
            // SETTING OFF
            else if (header.indexOf("GET /ManualOne") >= 0) {
              Serial.println("Manual Setting Off");
              for (int i = 0; i < ch; i++) {
                digitalWrite(relay[i], HIGH);
              }
              delay_seconds(RELAY_WAIT);
            ...
            // SETTING 1
            else if (header.indexOf("GET /ManualTwo") >= 0) {
              Serial.println("Manual Setting 1 off");
              for (int i = 0; i < ch; i++) {
                if (i == 0) {
                  digitalWrite(relay[i], LOW);
                }
            ...
            // SETTING 2
            else if (header.indexOf("GET /ManualThree") >= 0) {
              Serial.println("Manual Setting 2 on");
              for (int i = 0; i < ch; i++) {
                if (i == 2) {
                  digitalWrite(relay[i], LOW);
                }
            ...
            // SETTING 3
            else if (header.indexOf("GET /ManualFour") >= 0) {
              Serial.println("Manual Setting 3 on");
              for (int i = 0; i < ch; i++) {
                if (i == 1) {
                  digitalWrite(relay[i], LOW);
                }
            ...
            // Setting 4
            else if (header.indexOf("GET /ManualFive") >= 0) {
              Serial.println("Manual Setting 4 on");
              for (int i = 0; i < ch; i++) {
                if (i == 2) {
                  digitalWrite(relay[i], LOW);
                }
